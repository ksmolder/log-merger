...
public class LogMergerTest {
	...
	private void assertThatTimestampsAreAscending(String timestampFormat, Path outputPath, int expectedLineCount)
			throws IOException {
		Date lastDate = null;
		int lineCount = 0;
		SimpleDateFormat sdf = new SimpleDateFormat(timestampFormat);
		try (BufferedReader fileReader = new BufferedReader(
				new InputStreamReader(new FileInputStream(outputPath.toString()), "UTF-8"))) {
	...
	private void createBigFile(Path inputPath1, Path inputPath2, String timestampFormat, int numberOfLines)
			throws IOException {
		SimpleDateFormat sdf = new SimpleDateFormat(timestampFormat);
	...
	public void testMergeOneExceptionWithOneLine() throws IOException {
		String timestampFormat = "yyyy-MM-ddHH:mm:ss,SSS";
		Path outputPath = Paths.get(System.getProperty("user.dir"), "target",
				"testMergeOneExceptionWithNoException.log");
