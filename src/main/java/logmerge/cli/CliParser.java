		...
		try {
			...
			if (cli.hasOption("tf")) {
				...
				try {
					...
					throw new LogMergeException(LogMergeException.Reason.CliParser,
							"The provided timestamp format is not valid: " + e.getMessage(), e);
				}
			...
			if (cli.hasOption("f")) {
				...
				for (String part : parts) {
					try {
						...
						if (fieldNumber < 1) {
							throw new LogMergeException(LogMergeException.Reason.CliParser, "The field number '"
									+ fieldNumber + "' is too small. Please use an integer greater than 0.");
						}
						fields[fieldsIndex++] = fieldNumber;
					} catch (NumberFormatException e) {
						throw new LogMergeException(LogMergeException.Reason.CliParser,
								"Please provide an integer for option 'f': " + e.getMessage());
					}
				}
